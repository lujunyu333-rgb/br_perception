...
def coords(pt):
    ...
    try:
        c = pt.GetCoords()
        if c is not None:
            _M.add('GetCoords'); return [round(float(v), 4) for v in c]
    except Exception: pass
    try:
        r = [round(float(pt.X), 4), round(float(pt.Y), 4), round(float(pt.Z), 4)]
        _M.add('dyn.XYZ'); return r
    except Exception: pass
    try:
        sp = mod.ISketchPoint(pt._oleobj_)
        r = [round(float(sp.X), 4), round(float(sp.Y), 4), round(float(sp.Z), 4)]
        _M.add('typed.XYZ'); return r
    except Exception: pass
    _M.add('FAIL')
    return None
...
print('sketches:', len(out), 'segs:', tot, 'kinds:', kinds, '| methods:', _M or 'NONE')
